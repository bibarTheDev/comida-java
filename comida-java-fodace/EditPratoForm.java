...
class EditPratoForm extends JFrame implements ActionListener, MouseListener, DocumentListener
{
    //utilidades
    Banco db;
    Comida mainForm;
    ...
    JButton btUpdatePrato, btCancela;
     ...
     * 
     * @param form o formulario principal que chama esta
     * @param id o id do prato
     */
    EditPratoForm(Comida form, String id)
    {
        //basico
        ...
        btUpdatePrato = new JButton("Atualizar");
        btUpdatePrato.addActionListener(this);
        btUpdatePrato.setSize(160, 20);
        btUpdatePrato.setLocation(580, 410);
        this.add(btUpdatePrato);
    ...
    public void actionPerformed(ActionEvent ev)
    {
        ...
        if(ev.getSource() == btUpdatePrato){ updatePrato(); return; }
    ...
    public void updatePrato()
    {
        ...
        try{
            //insere o prato
            String sql = "UPDATE comida SET nome = '" + valores[0] + "', peso_volume = '" + valores[1] + "', descricao = '" + valores[2] + "' WHERE id = " + id + "; ";
    ...
    /**
     * carrega os valores do prato a ser mudado no form
     * 
     * @param id o id do prato
     */
    public void loadPratoData(String id)
    {
        try{
            //pega os dados
            String sql = "SELECT nome, peso_volume, descricao FROM comida WHERE id = " + id + ";";
            ArrayList< ArrayList<String> > prato = db.selectQuery(sql);
            
            //pega os ingredientes
            sql = "SELECT ingr FROM rel_ingredientes WHERE comida = " + id + ";";
            ArrayList< ArrayList<String> > pratoIngr = db.selectQuery(sql);

            //add as infos dos pratos
            txtNome.setText(prato.get(0).get(0));
            txtPesVol.setText(prato.get(0).get(1).split(" ", 0)[0]);
            txtDesc.setText(prato.get(0).get(2));

            //itera sobre ingredientes
            for(ArrayList<String> ingr : pratoIngr){
                //itera sobre tabela de possibilidades
                for(int i = 0; i < tbIngr.getRowCount(); i++) {
                    //add ingrediente
                    if((this.tbIngr.getValueAt(i, 0) + "").trim().equals(ingr.get(0))){
                        this.ingrList.add(this.tbIngr.getValueAt(i, 1) + "");
                    }
                }
            }
        } 
